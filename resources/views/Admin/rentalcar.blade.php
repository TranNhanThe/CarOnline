...
<hr>
<div class="container">
  <span class="word-ash-normal"><a class="word-ash" href=" {{ route('admin.rentallist') }} ">Xe thuê </a>-</span>  <span class="word-ash">{{$rentalcar->car_name}}</span> 
        ...
        @endforeach
          ...
          <div class="row bg-rentalcard rounded m-1 mb-4 p-4">
            ...
            <div class="col-md-4">
                ...
                @elseif (!$item->expiration_date && $item->status == 0 && $item->price)
                  <div class="bg-rentalcard word-green p-2 rounded">
                    <h2 class="mid"><i class='fa fa-warning'></i></h2>
                    <h3 class="mid">Đang chờ duyệt</h3>
                    <div class="mid">
                      <form method="post" action="{{ route('admin.status.toggle', $rentalcar->id) }}">
                        @csrf
                        @method('POST') 
                        @if ($item->status == 1)
               ...
               @elseif (!$item->expiration_date && !$item->status && !$item->price)
                  <div class="bg-rentalcard word-green p-2 rounded">
                  <h2 class="mid"><i class='fa fa-warning'></i></h2>
                  <h3 class="mid">chưa đăng tin</h3></div>
                @else
                <div class="bg-rentalcard word-green p-2 rounded">
                  <h2 class="mid"><i class='fa fa-warning'></i></h2>
                  <h3 class="mid">Chủ xe chưa gia hạn</h3></div>
                 @endif
                @endforeach
            </div>
            {{-- end info you --}}
